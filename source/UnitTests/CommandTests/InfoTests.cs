...
namespace Archiver.Tests
{
    public class InfoTests
    {
        ...
        [Fact]
        public void Execute_With_Valid_Archive_With_Multiple_Files_Returns_No_Error()
        {
            TestFileInformation[] files =
            {
                new TestFileInformation(
                ...
                new TestFileInformation(
                    "directory\\subdirectory\\third.txt",
                    300,
                    new byte[] { 8, 9 }),
            };
        ...
        [Fact]
        public void Execute_With_Truncated_Archive_Returns_Error()
        {
            string archivePath =
                Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".dat");
        ...
        private string CreateArchive(TestFileInformation[] files)
        {
            ...
            byte[] archiveIdentifier = Encoding.UTF8.GetBytes("ARCHIVE");

            writer.Write(archiveIdentifier);

            // Compression type.
            // Change this if NoCompress has another serialized value.
            writer.Write((uint)0);

            DateTime creationDate = new DateTime(2026, 9, 9, 12, 0, 0);
        ...
        private ParsedArguments CreateInfoArguments(string archivePath)
        {
            string[] arguments =
            {
                "-i",
